...
Purpose: Visual Search Popup for Matching score.
...
*/
	...
	require("visualsearch_slcand_setup.php");
	...
	{
		if($_GET['querystring']!=''){
			unset($_SESSION["SPHINX_MCCandidates"]);
		}
		//unset($_SESSION["SPHINX_MCCandidates"]);
	...
	if($_REQUEST['reset'] == 1){
		unset($_SESSION["SPHINX_MCCandidates"]);
		unset($_SESSION['SPHINX_MCCandidates_sub']);
	}
	$viewattr = $_GET['viewattr'];
	if(preg_match('/^(["\']).*\1$/m', $_GET['q'])) $_GET['q'] = "=".$_GET['q'];
	if(!empty($_SESSION["SPHINX_MCCandidates"]))
	{
		$searchstr = '';
		foreach($_SESSION['SPHINX_MCCandidates'] as $filtertype=>$filtername){	
			if(count($_SESSION['SPHINX_MCCandidates'][$filtertype])>0)
			{
				$searchstr .= "&".$filtertype."=";
				foreach($_SESSION['SPHINX_MCCandidates'][$filtertype] as $filters_id=>$filters_val)
				{
...
<title>Short List Candidates Attribute Search</title>
...
$(function() {
	...
	$("#attr-list .scroll-area").jScrollPane();
			...
			<?php
				...
				<?php }else if($viewattr == "employment_sdate"){//Search Filter for Completion Year
				?>
				...
				<?php }else if($viewattr == "employment_edate"){//Search Filter for Completion Year
				?>
			...
			?>		
					...
					$.ajax({
						url : 'savefilters_mc.php?cmdtype=view',
						dataType:  "jsonp",
						async: false,
	...
	$('.ckboxa').click(function(){
		...
		<?php
			...
			if((isset($_SESSION['SPHINX_MCCandidates_sub']['chkZipAll']) && $_SESSION['SPHINX_MCCandidates_sub']['chkZipAll'] !='') || (isset($_SESSION['SPHINX_MCCandidates_sub']['chkAreacodeAll'])) )
			{
				if(isset($_SESSION['SPHINX_MCCandidates_sub']['chkZipAll']) && $viewattr == 'zip')
				{
			?>
			...
			<?php }
			if(isset($_SESSION['SPHINX_MCCandidates_sub']['chkAreacodeAll']) && $viewattr == 'areacode'){?>
					document.getElementById('selrow').style='display:block;';
...
function modalBoxCloseandCancel()
{
	var pvstr = parent.top.$("input#SpeedSearchString").val();
		$.ajax({
			url : 'savefilters_mc.php?cmdtype=resetbynewchanges',
			dataType:  "jsonp",
			async: false,
			data: {
				q: pvstr,
...
function selectAll()
{
	...
	if(document.getElementById('<?php echo $viewattr;?>selecctall').checked==true)
	{
		...
		$.ajax({
			url : 'savefilters_mc.php?cmdtype=add'+chkall+'&searchtype='+searchtype+'&moduletype='+moduletype+'&'+formdata_selectall+actionStrAppend,
			type : 'post',
			dataType:  "jsonp",
			async: false,
			success : function(session)
	...
	else
	{
		...
		$.ajax({
			...
			url : 'savefilters_mc.php?cmdtype=update'+chkall+'&searchtype='+searchtype+'&moduletype='+moduletype+'&moduleid='+moduleid+'&modulevalue='+modulevalue,
			type : 'post',
			dataType:  "jsonp",
			async: false,
			success : function(session)
...
<div id="msg" class="VisualNewpopupM">
  <table width="730" cellpadding=1 cellspacing=1 border=0>
    <tr>
      <td valign=middle align="left" colspan="2" style="white-space: nowrap; padding-left:15px"><form method="get" name="search_form" id="search_form">
		  <input type="hidden" name="posid" value="<?=$_GET['posid'];?>" >
         ...
         <span class="visualNewSearch" onclick="return document.getElementById('search_form').submit();"><i class="fa fa-search fa-lg" type="submit" id="send" name="send" align="absmiddle" value="Search"  title="Search"></i></span>
          <span class="visualNewSearch" onclick="return window.location.href='visualsearch_popup_mc.php?viewattr=<?=$viewattr;?>&reset=1&querystring=<?=$_GET[querystring];?>';"><a href="visualsearch_popup_mc.php?posid=<?=$posid;?>&viewattr=<?=$viewattr;?>&reset=1&querystring=<?=$_GET['querystring'];?>" class="<?=$cssClas?>"><i class="fa fa-reply fa-lg" title="Reset"></i></a></span>
    ...
    <tr>
      <td class="VisualNewSortListHedBg" colspan="2">
      </td>
    </tr>
    <tr>
    ...
    <input type="hidden" name="notesopt" value="<?=$_GET['notesopt'];?>" >
    <input type="hidden" name="viewattr" value="<?=$viewattr;?>" >
    <input type="hidden" name="querystring" value="<?=$_GET['querystring'];?>" >
    <table width=100% cellpadding=1 cellspacing=1 border=0>
	 <tr><td align="left">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <input type="radio" name="nativesearch" value="I" <?php if(!isset($_GET['nativesearch'])) echo 'checked="checked"' ?> >Include in Search &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" name="nativesearch" value="E" <?php if($_GET['nativesearch']=='E') echo 'checked="checked"'; ?> >Exclude in Search &nbsp;</td><td align="right" style="padding-right:30px; display:none;" id="selrow"><div style="color:red;display:none;float:right;padding-top:4px;" id="maxSel20Alert">&nbsp;(* Limit 20)</div><div style="float:right;"><input type="checkbox" name="<?php echo $viewattr;?>selecctall" id="<?php echo $viewattr;?>selecctall" value="1"  onclick="javascript:selectAll();">Select/Deselect All</div></td></tr>
      <tr>
        <td colspan="2"><?php include_once("visualsearch_popupfilters_mc.php");?>
        </td>
      </tr>
      <tr>
...
<div class="acjbvisualsearchFilterCount">  
    <?php if($viewattr=="zip" || $viewattr=="areacode"){
			if($viewattr=="zip" ){
				if(isset($_SESSION['SPHINX_MCCandidates_sub']['savezipCODE']) && !empty($_SESSION['SPHINX_MCCandidates_sub']['savezipCODE']))
				{
					$zipcodesearch = $_SESSION['SPHINX_MCCandidates_sub']['savezipCODE'];
			...
			if($viewattr=="areacode"){
				if(isset($_SESSION['SPHINX_MCCandidates_sub']['saveareacodePSM']) && !empty($_SESSION['SPHINX_MCCandidates_sub']['saveareacodePSM']))
				{
					$areacodesearch = $_SESSION['SPHINX_MCCandidates_sub']['saveareacodePSM'];
	...
	if($viewattr == "edu_compdate" ){ //Search Filter for Completion Year
		...
		if(isset($_SESSION['SPHINX_MCCandidates_sub']['compyear']) && !empty($_SESSION['SPHINX_MCCandidates_sub']['compyear']))
		{
			$compyear = $_SESSION['SPHINX_MCCandidates_sub']['compyear'];
		...
		if($viewattr == "employment_sdate" ){ //Search Filter for Experience Start year
			...
			if(isset($_SESSION['SPHINX_MCCandidates_sub']['employment_sdate']) && !empty($_SESSION['SPHINX_MCCandidates_sub']['employment_sdate']))
			{
				$employment_sdate = $_SESSION['SPHINX_MCCandidates_sub']['employment_sdate'];
		...
		<?php } if($viewattr == "employment_edate" ){ //Search Filter for Experience End year
			...
			if(isset($_SESSION['SPHINX_MCCandidates_sub']['employment_edate']) && !empty($_SESSION['SPHINX_MCCandidates_sub']['employment_edate']))
			{
				$employment_edate = $_SESSION['SPHINX_MCCandidates_sub']['employment_edate'];
  ...
  <div class="acjbvisualFilterB">
    ...
    <?php } ?>
	 <?php if($viewattr=="amount"){ 
				if(isset($_SESSION['SPHINX_MCCandidates_sub']['saveamount']) && !empty($_SESSION['SPHINX_MCCandidates_sub']['saveamount']))
				{
					$saveamount = $_SESSION['SPHINX_MCCandidates_sub']['saveamount'];
...
function filtersApply()
{
	$(function(){
		...
		<?php } if($viewattr=="amount"){ ?>
			...
			var dateRangeSearch = "<?=$_GET['fmonth'];?>|<?=$_GET['fyear'];?>|<?=$_GET['tmonth'];?>|<?=$_GET['tyear'];?>";
			var actionStrAppend = "&compyear="+dateRangeSearch;
		<?php }if($viewattr=="employment_sdate"){//Search Filter for Experience Start Year?>
			var dateRangeSearch = "<?=$_GET['exps_month'];?>|<?=$_GET['exps_year'];?>|<?=$_GET['expe_month'];?>|<?=$_GET['expe_year'];?>";
			var actionStrAppend = "&emp_sdate="+dateRangeSearch;
		<?php }
		...
		<?php }?>	
			if($(this).is(':checked')==true)
			{
				...
				var modulevalue = encodeURIComponent($(this).parent().children('a').text());
				$.ajax({
					url : 'savefilters_mc.php?cmdtype=add'+chkall+'&searchtype='+searchtype+'&moduletype='+moduletype+'&moduleid='+moduleid+'&modulevalue='+modulevalue+actionStrAppend,
					dataType:  "jsonp",
					async: false,
					success : function(session)
					{
			...
			}else if($(this).is(':checked')==false)
			{
				...
				$.ajax({
					url : 'savefilters_mc.php?cmdtype=update'+chkall+'&searchtype='+searchtype+'&moduletype='+moduletype+'&moduleid='+moduleid+'&modulevalue='+modulevalue+'&uncheck=1',
					dataType:  "jsonp",
					async: false,
					success : function(session)
					{
...
{
	$.ajax({
		url : 'savefilters_mc.php?cmdtype=update&moduletype='+moduletype+'&moduleid='+moduleid,
		dataType:  "jsonp",
		async: false,
		success : function(session)
		{
	...
	function() {
	    $("#suggest").autocomplete(
			...
			source : function(request, response) {
			    $.ajax({
				url : 'visualsearch_searchsuggest_sl.php?viewattr=<?=$_GET['viewattr'];?>&querystring=<?=$_GET['querystring'];?>',
				dataType : 'json',
				data : {
				    term : request.term
				},
...
function remFilters(moduletype,moduleid)
{
	...
	?>
	$.ajax({
		url : 'savefilters_mc.php?cmdtype=update'+chkall+'&moduletype='+moduletype+'&moduleid='+moduleid,
		dataType:  "jsonp",
		async: false,
		success : function(session)
		{
...
function remAllFilters(filterType)
{
	...
	?>
	$.ajax({
		url : 'savefilters_mc.php?cmdtype=delete&moduletype='+filterType,
		dataType:  "jsonp",
		async: false,
		success : function(session)
		{
