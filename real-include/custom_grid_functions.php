        ...
        function queryBuilder($modulename, $userid, $cvsid=0)
        {
			...
			else
			{
                ...
                if($modulename == 'contacts'){
                	...
                	$deptAccessSnos = $deptAccessObj->getDepartmentAccess($username,"'FO'");

                    $sel_grid_content .= " AND ".$mod_details['primary_table'].".status= 'ER' AND (FIND_IN_SET('".$username."',".$mod_details['primary_table'].".accessto)>0 OR ".$mod_details['primary_table'].".owner='".$username."' OR ".$mod_details['primary_table'].".accessto='ALL') AND ".$mod_details['primary_table'].".deptid !='0' AND ".$mod_details['primary_table'].".deptid IN (".$deptAccessSnos.") ";
                }
                if($modulename == 'companies'){
                	$deptAccessObj = new departmentAccess();
                	$deptAccessSnos = $deptAccessObj->getDepartmentAccess($username,"'FO'");

                    $sel_grid_content .= " AND ".$mod_details['primary_table'].".status= 'ER' AND (".$mod_details['primary_table'].".owner = '".$username."' OR FIND_IN_SET( '".$username."', ".$mod_details['primary_table'].".accessto ) >0 OR ".$mod_details['primary_table'].".accessto = 'ALL') AND ".$mod_details['primary_table'].".crmcompany='Y' AND ".$mod_details['primary_table'].".deptid !='0' AND ".$mod_details['primary_table'].".deptid IN (".$deptAccessSnos.") ";
                }
                if($modulename == 'candidates'){
                	$deptAccessObj = new departmentAccess();
                	$deptAccessSnos = $deptAccessObj->getDepartmentAccess($username,"'FO'");

                 	$sel_grid_content .= " AND ".$mod_details['primary_table'].".status= 'ACTIVE'  AND (".$mod_details['primary_table'].".owner='".$username."' OR FIND_IN_SET('".$username."',".$mod_details['primary_table'].".accessto )>0 OR ".$mod_details['primary_table'].".accessto='ALL') AND ".$mod_details['primary_table'].".deptid !='0' AND ".$mod_details['primary_table'].".deptid IN (".$deptAccessSnos.") ";
                }
                if($modulename == 'job orders'){
                	$deptAccessObj = new departmentAccess();
                	$deptAccessSnos = $deptAccessObj->getDepartmentAccess($username,"'FO'");

                    $sel_grid_content .= " AND (".$mod_details['primary_table'].".owner='".$username."' OR FIND_IN_SET('".$username."',".$mod_details['primary_table'].".accessto)>0 OR ".$mod_details['primary_table'].".accessto='all') AND ".$mod_details['primary_table'].".status IN ('approve','Accepted') AND ".$mod_details['primary_table'].".deptid !='0' AND ".$mod_details['primary_table'].".deptid IN (".$deptAccessSnos.") ";
                }
