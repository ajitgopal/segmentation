...
function displayTimesheetDetailsPrint($sno, $mode,$condinvoice ='',$conjoin='', $module='', $print=True, $invoice='',$ts_type='',$Cval='')
{
   ...
   function displayTimesheetDetails($sno, $mode, $condinvoice ='', $conjoin='', $module='', $invoice='',$ts_type='',$Cval='')
    ...
    function getDetailsBySnos($snos, $ts_type = '', $mode = '',$module='')
    {
	$ratetimedata	= "";
	...
	if (!empty($snos))
	{
		...
		$sel_sno_query	= "SELECT
						th.parid, ".$col1.", th.hourstype as rate, th.billable as billable ".$ts_notes.",th.status as ts_status,th.rowid as ts_rowid,th.sno as ts_sno
					FROM
		...
		if (mysql_num_rows($res_sno_query) > 0) {
			...
			while ($row_rate_query = $this->mysqlobj->fetch_object($res_sno_query)) {
				...
				if($module == 'MyProfile' && $row_rate_query->ts_status == 'Saved'){
					$rate_data	= $row_rate_query->rate."|".$row_rate_query->hours."|".$row_rate_query->billable."|".$row_rate_query->ts_status."|".$row_rate_query->ts_rowid."|".$row_rate_query->ts_sno;
				}else{
