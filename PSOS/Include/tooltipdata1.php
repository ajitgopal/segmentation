...
{
	parent.info_pass(val,fcomp,contactname,accsno,comp_sno,comp_name);
}
...
{ 
	...
	if($acc_rows>0)
	{
		...
		{ 
			...
			$CompName=str_replace('"','|Akkendbquote|',$dd1[5]);
			...
			<td colspan="5" height="22" onclick="win('<?=addslashes($dd1[0])?>','<?=addslashes($fcomp)?>','<?=addslashes($contname)?>','<?=addslashes($dd1[2])?>','<?php echo addslashes($dd1[4]);?>','<?php echo addslashes($CompName);?>')"> 
			<b><? echo $dd1[1]; if($dd1[5]!='') echo "(".stripslashes($dd1[5]).") "; ?></b></td>
			</tr>
			<tr nowrap="nowrap">
	...
	if($num_rows > 0) 
	{
		...
		{ 
			...
			<tr><input type="hidden" name=comp<?=$dd1[0]?> value="<? echo dispfdb($dd1[1]); ?>" /></tr>
			<tr onmouseover="company('<?=$q?>')" onMouseOut="company_out(<?=$q?>)" onMouseDown="company_out('<?=$q?>')" id=com<?=$q?> class="mouseoutcont">
			<td colspan="5" height="22" onclick="win('<?=addslashes($dd1[0])?>','<?=addslashes($fcomp)?>','<?=addslashes($contname)?>','','<?=addslashes($dd1[3])?>','<?php echo addslashes($CompName);?>')"> 
			<b><? echo $dd1[1]; if($dd1[4]!='') echo  "(".stripslashes($dd1[4]).") "; echo $dd1[2]; ?></b></td>
			</tr>
			<tr nowrap="nowrap">
				<td colspan="5" bgcolor="#ffffff"></td>
			</tr>
	...
	{   
		...
		for($i=0 ; $i<10;$i++)
		{
			...
			{  
				...
				<tr><input type="hidden" name=comp<?=$dd1[0]?> value="<? echo dispfdb($dd1[1]); ?>" /></tr>						
				<tr onmouseover="company('<?=$q?>')" onMouseOut="company_out('<?=$q?>')" onMouseDown="company_out('<?=$q?>')" id=com<?=$q?> class="mouseoutcont">
				<td colspan="5" height="22" onclick="win('<?=addslashes($dd1[0])?>','<?=addslashes($fcomp)?>','<?=addslashes($contname)?>','<?=addslashes($dd1[2])?>','<?=addslashes($dd1[4])?>','<?php echo addslashes($CompName);?>')"> 
				<b><? echo $dd1[1]; ?></b> </td>
				</tr>
				<tr nowrap="nowrap">
					<td colspan="5" bgcolor="#ffffff"></td>
	...
	else
	{
		...
		if($acc_rows>0)
		{
			...
			{ 
				...
				$CompName=str_replace("'",'|Akkensiquote|',$CompName);
				$dd1[5] = dispfdb(stripslashes($dd1[5]));
				$dd1[1] = dispfdb(stripslashes($dd1[1]));//contact name
				?>
				<tr><input type="hidden" name=comp<?=$dd1[0]?> value="<? echo dispfdb($dd1[1]); ?>" /></tr>
				<tr onmouseover="company('<?=$q?>')" onMouseOut="company_out(<?=$q?>)" onMouseDown="company_out('<?=$q?>')" id=com<?=$q?> class="mouseoutcont">
				<td colspan="5" height="22" onclick="win('<?=addslashes($dd1[0])?>','<?=addslashes($fcomp)?>','<?=addslashes($contname)?>','<?=addslashes($dd1[2])?>','<?php echo addslashes($dd1[4]);?>','<?php echo addslashes($CompName);?>')"> 
