<?php
	...
	if($empnames=="")
	{
		...
		else
		{
			...
			FROM emp_list, hrcon_jobs, hrcon_compen,department
			WHERE emp_list.username = hrcon_jobs.username AND department.sno = hrcon_compen.dept AND emp_list.username = hrcon_compen.username AND emp_list.lstatus != 'DA' AND emp_list.lstatus != 'INACTIVE' AND (emp_list.empterminated != 'Y' || (UNIX_TIMESTAMP(DATE_FORMAT(emp_list.tdate,'%Y-%m-%d'))-UNIX_TIMESTAMP())>0) AND hrcon_jobs.ustatus IN ('active','closed','cancel') AND hrcon_compen.ustatus != 'backup' AND hrcon_jobs.jtype != '' AND hrcon_compen.timesheet != 'Y' AND department.sno !='0' AND department.sno IN(".$deptAccesSno.")
	...
	FROM emp_list, hrcon_jobs, hrcon_compen,hrcon_general,department
	...
	AND department.sno = hrcon_compen.dept
	...
	//query for getting employee list for to fill time sheet 
	$query="SELECT emp_list.username uid, CONCAT_WS(' ',hrcon_general.lname,hrcon_general.fname,hrcon_general.mname), ".getEntityDispName("CONCAT_WS(' ',hrcon_general.lname,hrcon_general.fname,hrcon_general.mname)","emp_list.sno").", hrcon_jobs.assign_no
	FROM emp_list, hrcon_jobs, hrcon_compen,hrcon_general,department
	WHERE emp_list.username = hrcon_jobs.username
	AND emp_list.username = hrcon_compen.username
	AND emp_list.username = hrcon_general.username
	...
	ORDER BY trim(hrcon_general.lname),trim(hrcon_general.fname),trim(hrcon_general.mname)";
	$result=mysql_query($query,$db);
	$enames="<option value=''> -- Employee Name -- </option>";
	
	$asgnCounter = 0;
	while($myrow=mysql_fetch_row($result))
	{
		// BELOW QUERY IS NOT USED IN THIS LOOP - PAVAN
		/*
		$getmaxdate = "SELECT MAX(edate) FROM par_timesheet WHERE username='".$myrow[0]."'";
		$maxres=mysql_query($getmaxdate,$db);
		$maxdaterow=mysql_fetch_row($maxres);

		// MOVED THE BELOW CONDITION TO THE ABOVE QUERY
		$dynamicUstatus = " AND hrcon_jobs.jtype!='' AND ((hrcon_jobs.ustatus IN ('active','closed','cancel') AND (hrcon_jobs.s_date IS NULL OR hrcon_jobs.s_date='' OR hrcon_jobs.s_date='0-0-0' OR (DATE(STR_TO_DATE(s_date,'%m-%d-%Y')) <= '".$checking_to."'))) AND (IF(hrcon_jobs.ustatus='closed',(hrcon_jobs.e_date IS NOT NULL AND hrcon_jobs.e_date<>'' AND hrcon_jobs.e_date<>'0-0-0' AND DATE(STR_TO_DATE(e_date,'%m-%d-%Y'))>='".$checking_from."'),1)) AND (IF(hrcon_jobs.ustatus='cancel',(hrcon_jobs.e_date IS NOT NULL AND hrcon_jobs.e_date<>'' AND hrcon_jobs.e_date<>'0-0-0' AND hrcon_jobs.e_date <> hrcon_jobs.s_date AND DATE(STR_TO_DATE(e_date,'%m-%d-%Y'))>='".$checking_from."'),1)))";
		$getActiveAssignments = "select count(1) from hrcon_jobs where username = '".$myrow[0]."' and pusername!=''".$dynamicUstatus." order by udate";
		$activeRes=mysql_query($getActiveAssignments,$db);
		$activeCount=mysql_fetch_row($activeRes);
		...
		*/
			...
			if($enames=="")
			$enames="<option value='".$myrow[0]."'  title='".html_tls_specialchars($myrow[1],ENT_QUOTES)."'>".$str_name."</option>";
			else
			$enames.="<option value='".$myrow[0]."' title='".html_tls_specialchars($myrow[1],ENT_QUOTES)."'>".$str_name."</option>";
		$asgnCounter++;
		//if($asgnCounter == $totalAsgnArr[$myrow[0]])
		//{
		//	$enames.="<option value='".$myrow[0]."' title='".html_tls_specialchars($myrow[1],ENT_QUOTES)."'>".html_tls_specialchars($myrow[2], ENT_QUOTES)." (Benefits)</option>";
		//	$asgnCounter = 0;
		//}
		...
		if($asgnCounter == $totalAsgnArr[$myrow[0]])
		{
			...
			if(mysql_num_rows($res1) > 0)
			{
				...
				while($r=mysql_fetch_array($res1)) {
                 ...
                 $enames.="<option value='".$myrow[0]."' title='".html_tls_specialchars($myrow[1],ENT_QUOTES)."'>".html_tls_specialchars($myrow[2], ENT_QUOTES)." - ".$r[0]." </option>";
				}
			}
			$asgnCounter = 0;
		}
...
$dynamicUstatus = " AND hrcon_jobs.jtype!='' AND ((hrcon_jobs.ustatus IN ('active','closed','cancel') AND (hrcon_jobs.s_date IS NULL OR hrcon_jobs.s_date='' OR hrcon_jobs.s_date='0-0-0' OR (DATE(STR_TO_DATE(s_date,'%m-%d-%Y')) <= '".$checking_to."'))) AND (IF(hrcon_jobs.ustatus='closed',(hrcon_jobs.e_date IS NOT NULL AND hrcon_jobs.e_date<>'' AND hrcon_jobs.e_date<>'0-0-0' AND DATE(STR_TO_DATE(e_date,'%m-%d-%Y'))>='".$checking_from."'),1)) AND (IF(hrcon_jobs.ustatus='cancel',(hrcon_jobs.e_date IS NOT NULL AND hrcon_jobs.e_date<>'' AND hrcon_jobs.e_date<>'0-0-0' AND hrcon_jobs.e_date <> hrcon_jobs.s_date AND DATE(STR_TO_DATE(e_date,'%m-%d-%Y'))>='".$checking_from."'),1))) AND hrcon_compen.username = hrcon_jobs.username AND hrcon_compen.dept IN ($deptnos) ";
			
			$getActiveAssignments = "select count(1),CONCAT_WS(' ',hrcon_general.lname,hrcon_general.fname,hrcon_general.mname,hrcon_general.username) from hrcon_jobs,  hrcon_general, hrcon_compen where hrcon_general.username = hrcon_jobs.username AND hrcon_general.ustatus IN ('active','ACTIVE') AND hrcon_jobs.username = '".$elementsbefore[$i]."' and  hrcon_jobs.pusername!=''".$dynamicUstatus." AND hrcon_compen.ustatus!= 'backup' GROUP BY hrcon_jobs.username";
	...
	<table width=100% cellpadding=0 cellspacing=0 border=0  align="center" class="ProfileNewUI defaultTopRange">
	 <div id="topheader" width="100%">
		...
		<tr>
				...
				<td valign="middle"><font style="vertical-align:middle" class=lfontstyle4>&nbsp;&nbsp;&nbsp;&nbsp;<strong>Select an Employee:</strong>&nbsp;&nbsp;&nbsp;&nbsp;</font><select name="empnames" id="empnames" class=drpdwnaccemp tabindex="3"><?php echo $enames; ?></select><span id='TodoAdd' class="multiAddBtn" style="cursor:pointer"  onclick="javascript:addNewEmployee()" ><i class="fa fa-plus-square"></i> Add</span></td><td valign="middle"> </td><td>&nbsp;</td><td align="left"><font style="vertical-align:middle" class=lfontstyle4><strong>No. of Employees selected : <?php echo $numberOfEmployeeds;?></strong></font></td></tr></table>
				...
				for($r=0;$r<$rowcou;$r++)
				{
					...
					$tab_index_task = $tab_index+5;
						...
						<tr class=<?php echo $class;?>>
							...
							$assignOptions="";
							$uom_query ='';
							$dynamicUstatus = " AND hrcon_jobs.jtype!='' AND ((hrcon_jobs.ustatus IN ('active','closed','cancel') AND (hrcon_jobs.s_date IS NULL OR hrcon_jobs.s_date='' OR hrcon_jobs.s_date='0-0-0' OR (DATE(STR_TO_DATE(s_date,'%m-%d-%Y')) <= '".$checking_to."'))) AND (IF(hrcon_jobs.ustatus='closed',(hrcon_jobs.e_date IS NOT NULL AND hrcon_jobs.e_date<>'' AND hrcon_jobs.e_date<>'0-0-0' AND DATE(STR_TO_DATE(e_date,'%m-%d-%Y'))>='".$checking_from."'),1)) AND (IF(hrcon_jobs.ustatus='cancel',(hrcon_jobs.e_date IS NOT NULL AND hrcon_jobs.e_date<>'' AND hrcon_jobs.e_date<>'0-0-0' AND hrcon_jobs.e_date <> hrcon_jobs.s_date AND DATE(STR_TO_DATE(e_date,'%m-%d-%Y'))>='".$checking_from."'),1))) AND hrcon_compen.dept IN ($deptnos)";
                             
			     				$zque = "select hrcon_jobs.sno, hrcon_jobs.client, hrcon_jobs.project, hrcon_jobs.jtype, hrcon_jobs.pusername, hrcon_jobs.jotype, hrcon_jobs.classid, date_format(str_to_date(s_date,'%m-%d-%Y'),'%m/%d/%Y'), date_format(str_to_date(e_date,'%m-%d-%Y'),'%m/%d/%Y'), assign_no,multi_rates.mulrates from hrcon_jobs LEFT JOIN hrcon_compen ON hrcon_jobs.username = hrcon_compen.username
			     						left join (select asgnid,GROUP_CONCAT( CONCAT_WS( '^^', ratemasterid, period ) SEPARATOR '&&' ) as mulrates from `multiplerates_assignment` where ratetype = 'billrate' and asgn_mode = 'hrcon' and STATUS = 'Active' GROUP BY asgnid) multi_rates ON multi_rates.asgnid=hrcon_jobs.sno  where $condCk_comp hrcon_jobs.username = '".$empfetchdata."'  and hrcon_jobs.pusername!=''".$dynamicUstatus." $showEmplyoees AND hrcon_compen.ustatus!='backup' order by hrcon_jobs.udate DESC";
