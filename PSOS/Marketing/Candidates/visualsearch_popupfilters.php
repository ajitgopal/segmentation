...
*/
	...
	if(!empty($_GET['query']) && $_GET['query']!=''){
		...
		if($qu1!=''){
			...
			{				
				...
				//$qu2 = preg_replace('/[^\w~\|\(\)\^\$\?"\/=-]+/',' ',trim(strtolower($qu2)));
	...
	//echo $r;
			...
			if(isset($_SESSION['SPHINX_Candidates_sub']['savezipCODE']) && $_SESSION['SPHINX_Candidates_sub']['savezipCODE'] !=''){
				$zipCode_all = explode('|',$_SESSION['SPHINX_Candidates_sub']['savezipCODE']);
			}
			if(isset($_SESSION['SPHINX_Candidates_sub']['saveareacodePSM']) && $_SESSION['SPHINX_Candidates_sub']['saveareacodePSM'] !=''){
				$areaCode_all = explode('|',$_SESSION['SPHINX_Candidates_sub']['saveareacodePSM']);
			}
			 if(($_GET['radiuszip']!='' && $_GET['zipmiles']!='' && $_GET['go'] == 'Search') || ($zipCode_all[0] !='' && $zipCode_all[1] !=''))
			 {
				...
				if($zipcode!='')
				{
					...
					// Checking whether zipcode radius search is exclude 
					if(isset($_SESSION['SPHINX_Candidates']['zip']))
					{
						foreach($_SESSION['SPHINX_Candidates']['zip'] as $zipsearchkey => $zipsearchval) 
						{
						    if("E|"==substr($zipsearchkey,0,2)) 
						    {
						        $zipstr_where = " AND distance > {$meters} ";
						        break;
						    }
						    else if("I|"==substr($zipsearchkey,0,2))
						    {
						    	break;
						    }
						}
			...
			if($attr=="edu_compdate"){//Search Filter for Completion Year
				...
				if($_GET['fmonth'] !='' && $_GET['fyear'] !='' && $_GET['tmonth'] !='' && $_GET['tyear'] !='' && ( $_GET['go']=='Search' || !empty($_SESSION['SPHINX_Candidates_sub']['compyear']))){
			...
			if($attr=="employment_sdate"){//Search Filter for Experience Start Date
				...
				if($_GET['exps_month'] !='' && $_GET['exps_year'] !='' && $_GET['expe_month'] !='' && $_GET['expe_year'] !='' && ( $_GET['go']=='Search' || !empty($_SESSION['SPHINX_Candidates_sub']['employment_sdate']))){
			...
			if($attr=="employment_edate"){//Search Filter for Experience End Date
				...
				if($_GET['expend_smonth'] !='0' && $_GET['expend_syear'] !='0' && $_GET['expend_emonth'] !='0' && $_GET['expend_emonth'] !='Present' && $_GET['expend_eyear'] !='0' && ( $_GET['go']=='Search' || !empty($_SESSION['SPHINX_Candidates_sub']['employment_edate']))){
			...
			if($_GET['query']!=''){
				...
				if(in_array($attr,$subArray)){
					...
					if($attr=="edu_country")
					{
						...
						$query = "SELECT @groupby,{$attr}, COUNT(*) AS cnt FROM {$SPHINX_CONF['sphinx_index']}  WHERE crc_accessto IN (2914988887,{$username}) AND {$attr} IN ({$countryS}) GROUP BY {$attr} ORDER BY {$attr} ASC LIMIT {$currentOffset},{$SPHINX_CONF['page_size']} {$optionsList}";
					} else
					...
					if($attr=="contact_method")
					{
						...
						$query = "SELECT @groupby,{$attr}, COUNT(*) AS cnt, weight() AS w FROM {$SPHINX_CONF['sphinx_index']} WHERE crc_accessto IN (2914988887,{$username}) {$cMethod} GROUP BY {$attr} ORDER BY w DESC LIMIT 0,10 OPTION ranker=sph04";
					} else
					if($attr=="cre_type")
					{
						$sub='';
						...
						$query = "SELECT @groupby,{$attr}, COUNT(*) AS cnt FROM {$SPHINX_CONF['sphinx_index']}  WHERE crc_accessto IN (2914988887,{$username}) AND {$attr} IN ({$cretypeS}) GROUP BY {$attr} ORDER BY {$attr} ASC LIMIT {$currentOffset},{$SPHINX_CONF['page_size']} {$optionsList}";
					} else					
					if($attr=="cre_name")
					{
						$sub='';
						...
						$query = "SELECT @groupby,{$attr}, COUNT(*) AS cnt FROM {$SPHINX_CONF['sphinx_index']}  WHERE crc_accessto IN (2914988887,{$username}) AND {$attr} IN ({$crenameS}) GROUP BY {$attr} ORDER BY {$attr} ASC LIMIT {$currentOffset},{$SPHINX_CONF['page_size']} {$optionsList}";
					} else
					if($attr=="cre_number")
					{
						$sub='';
						...
						$query = "SELECT @groupby,{$attr}, COUNT(*) AS cnt FROM {$SPHINX_CONF['sphinx_index']}  WHERE crc_accessto IN (2914988887,{$username}) AND {$attr} IN ({$crenumberS}) GROUP BY {$attr} ORDER BY {$attr} ASC LIMIT {$currentOffset},{$SPHINX_CONF['page_size']} {$optionsList}";
					} else
					if($attr=="availsdate")
					{
						$sub='';
						...
						$query = "SELECT @groupby,{$attr}, COUNT(*) AS cnt FROM {$SPHINX_CONF['sphinx_index']}  WHERE crc_accessto IN (2914988887,{$username}) AND {$attr} IN ({$creacqS}) GROUP BY {$attr} ORDER BY {$attr} ASC LIMIT {$currentOffset},{$SPHINX_CONF['page_size']} {$optionsList}";
					} else
					...
					if($attr=="cre_validfrom")
					{
						...
						$query = "SELECT @groupby,{$attr}, COUNT(*) AS cnt FROM {$SPHINX_CONF['sphinx_index']}  WHERE crc_accessto IN (2914988887,{$username}) AND {$attr} IN ({$crefromS}) GROUP BY {$attr} ORDER BY {$attr} ASC LIMIT {$currentOffset},{$SPHINX_CONF['page_size']} {$optionsList}";
					} else
					if($attr=="cre_validto")
					{
						$sub='';
						...
						$query = "SELECT @groupby,{$attr}, COUNT(*) AS cnt FROM {$SPHINX_CONF['sphinx_index']}  WHERE crc_accessto IN (2914988887,{$username}) AND {$attr} IN ({$cretoS}) GROUP BY {$attr} ORDER BY {$attr} ASC LIMIT {$currentOffset},{$SPHINX_CONF['page_size']} {$optionsList}";
					}else
					if($attr=="cre_state")
					{
						$sub='';
				...
				}else
				{
					 if($attr=="areacode")
					 {
						$query = "SELECT @groupby,hareacode ,wareacode,mareacode, COUNT(*) AS cnt FROM {$SPHINX_CONF['sphinx_index']}  WHERE MATCH('{$r}') AND crc_accessto IN (2914988887,{$username}) GROUP BY hareacode,wareacode,mareacode ORDER BY hareacode ASC LIMIT {$currentOffset},{$SPHINX_CONF['page_size']} {$optionsList}";
						...
						if(count($matchingRecords) > 0)
						{
							...
							$query = "SELECT @groupby,IF(crc_candtype=2766739447,2766739447,if(owner={$username},189759857,577727202)) as crc_candtype1, COUNT(*) AS cnt FROM {$SPHINX_CONF['sphinx_index']}  WHERE MATCH('{$r}') AND crc_accessto IN (2914988887,{$username}) AND crc_candtype1 IN ({$candAttributesStr}) GROUP BY crc_candtype1 ORDER BY cnt ASC LIMIT {$currentOffset},{$SPHINX_CONF['page_size']} {$optionsList}";
						}
						else
						{
							$query = "SELECT @groupby,IF(crc_candtype=2766739447,2766739447,if(owner={$username},189759857,577727202)) as crc_candtype1, COUNT(*) AS cnt FROM {$SPHINX_CONF['sphinx_index']}  WHERE MATCH('') AND crc_accessto IN (2914988887,{$username}) AND crc_candtype1 IN (0) GROUP BY crc_candtype1 ORDER BY cnt ASC LIMIT {$currentOffset},{$SPHINX_CONF['page_size']} {$optionsList}";
						}						
					 }else
					 {
				...
				}else if($q3!='')
				{
					...
					if($attr=="areacode")
					{
						$query = "SELECT @groupby,hareacode,wareacode,mareacode ,COUNT(*) AS cnt {$zipstr} {$areacodestr} FROM {$SPHINX_CONF['sphinx_index']}  WHERE {$q3} AND crc_accessto IN (2914988887,{$username}) {$zipstr_where} {$areastrWhere} GROUP BY hareacode,wareacode,mareacode ORDER BY hareacode ASC LIMIT {$currentOffset},{$SPHINX_CONF['page_size']} {$optionsList}";
						
						
					}else if($attr=="crc_candtype"){
						$query = "SELECT @groupby,IF(crc_candtype=2766739447,2766739447,if(owner={$username},189759857,577727202)) as crc_candtype1, COUNT(*) AS cnt {$zipstr} {$areastr} FROM {$SPHINX_CONF['sphinx_index']}  WHERE {$q3} AND crc_accessto IN (2914988887,{$username}) {$zipstr_where} {$areastr_where} GROUP BY crc_candtype1 ORDER BY {$orderby} LIMIT {$currentOffset},{$SPHINX_CONF['page_size']} {$optionsList}";
					 }else
					 {
				...
				}else
				{
					if($attr=="areacode")
					{
						$query = "SELECT @groupby,hareacode,wareacode,mareacode, COUNT(*) AS cnt {$zipstr} {$areacodestr} FROM {$SPHINX_CONF['sphinx_index']}  WHERE crc_accessto IN (2914988887,{$username}) {$zipstr_where} {$areastrWhere} GROUP BY hareacode,wareacode,mareacode ORDER BY hareacode ASC LIMIT {$currentOffset},{$SPHINX_CONF['page_size']} {$optionsList}";
						
						
					}else if($attr=="crc_candtype"){
					
							$query = "SELECT @groupby,IF(crc_candtype=2766739447,2766739447,if(owner={$username},189759857,577727202)) as crc_candtype1, COUNT(*) AS cnt {$zipstr} {$areastr} FROM {$SPHINX_CONF['sphinx_index']}  WHERE crc_accessto IN (2914988887,{$username}) {$zipstr_where} {$areastr_where} GROUP BY crc_candtype1 ORDER BY {$orderby} LIMIT {$currentOffset},{$SPHINX_CONF['page_size']} {$optionsList}";
					}else if($attr=="amount" && ($minsal!='' || $maxsal!='') ){
						
							$query = "SELECT @groupby,{$attr}, COUNT(*) AS cnt {$zipstr} {$areastr} FROM {$SPHINX_CONF['sphinx_index']}  WHERE crc_accessto IN (2914988887,{$username})";
							if($minsal!='' && $maxsal!='')
							{
						...
						if($SPHINX_CONF['sphinx_attributes'][$attr]=="numeric")
						{
							...
							if(in_array($attr,$subArray)){
									...
									if($attr=="skills" && $attrsIds!=''){
										...
										$skillSelTypeAry = array();
										$skillSelTypeAry = explode(",", $skillSelTypeStr);
										$skillTypeCond = "";
										if (count($skillSelTypeAry)>0) {

											$s_t_query = "SELECT mvalue FROM candidate_master WHERE mtype='skilltype' AND sno IN(".$skillSelTypeStr.")";
											$resSkilsType=mysql_query($s_t_query,$db);
											if (mysql_num_rows($resSkilsType)==1) {
												$rowSkillsType=mysql_fetch_row($resSkilsType);
												if ($rowSkillsType[0]=="Managed") {
													$skillTypeCond = "AND b.skillname != '' AND b.manage_skills_id<>0";
												}elseif ($rowSkillsType[0]=="Parsed") {
													$skillTypeCond = "AND b.skillname != '' AND b.manage_skills_id=0";
												}
											}
										}
										$queSkilsType="select DISTINCT a.sno, a.mvalue AS skillname, a.sno AS snoIDs FROM candidate_master a
											INNER JOIN candidate_skills b ON (b.skillname = a.mvalue)
											WHERE a.sno IN({$attrsIds}) AND a.mtype='skillname' ".$skillTypeCond."  ORDER BY a.mvalue ASC,FIELD(a.sno, {$attrsIds}) LIMIT 0,{$SPHINX_CONF['page_size']}";
										...
										while($rowSkillsType=mysql_fetch_assoc($resSkilsType))
										{
											...
											if(isset($_SESSION['SPHINX_Candidates'][$attr][$rowSkillsType['snoIDs']])){ 
												$checked = 'checked="checked"' ; 
											}
										...
										while($rowSkillsLevel=mysql_fetch_assoc($resSkilsType))
										{
											...
											$checked = '';
												...
												if(isset($_SESSION['SPHINX_Candidates'][$attr][$rowSkillsLevel['snoIDs']])){ 
													$checked = 'checked="checked"' ; 
												}
										...
										while($rowSkillsLastused=mysql_fetch_assoc($resSkilsType))
										{
											...
											$checked = '';
												...
												if(isset($_SESSION['SPHINX_Candidates'][$attr][$rowSkillsLastused['snoIDs']])){ 
													$checked = 'checked="checked"' ; 
												}
										...
										while($rowEduType=mysql_fetch_assoc($resEduType))
										{
											...
											$checked = '';
												...
												if(isset($_SESSION['SPHINX_Candidates'][$attr][$rowEduType['snoIDs']])){ 
													$checked = 'checked="checked"' ; 
												}
											...
											foreach($compDate as $date){
												if($comp_date_res['to_days'] !='' && $compto_date_res['to_days']!='' && $comp_date_res['to_days'] <=$date && $compto_date_res['to_days'] >=$date){
													...
													if($date!=0)
													{
														...
														$checked = '';
														if(in_array($print_value,$_SESSION['SPHINX_Candidates'][$attr]) || in_array($date,$_SESSION['SPHINX_Candidates'][$attr])){ 
															$checked = 'checked="checked"' ; 
														}
										...
										while($rowEmpType=mysql_fetch_assoc($resEmpType))
										{
											...
											if(isset($_SESSION['SPHINX_Candidates'][$attr][$rowEmpType['snoIDs']])){ 
													$checked = 'checked="checked"' ; 
												}
										...
										while($rowEmpType=mysql_fetch_assoc($resEmpType))
										{
											...
											foreach ($attrsNewArray as $idx => $row)
											{
												...
												if(isset($_SESSION['SPHINX_Candidates'][$attr][$cm])){ 
													$checked = 'checked="checked"' ; 
												}
										...
										while($rowcreType=mysql_fetch_assoc($rescreType))
										{
												...
												if(isset($_SESSION['SPHINX_Candidates'][$attr][$rowcreType['snoIDs']])){ 
													$checked = 'checked="checked"' ; 
												}
									...
									{	
										...
										foreach($states_creds as $states_creds_key=>$states_creds_val){
											...
											if(isset($_SESSION['SPHINX_Candidates'][$attr][$states_creds_val[0]])){ 
												$checked = 'checked="checked"' ; 
											}
									...
									while($rowroleType=mysql_fetch_assoc($resroleType))
									{
											...
											if(isset($_SESSION['SPHINX_Candidates'][$attr][$rowroleType['snoIDs']])){ 
												$checked = 'checked="checked"' ; 
											}
								...
								{									
									...
									while($rowrolePer=mysql_fetch_assoc($resrolePer))
									{
											...
											if(isset($_SESSION['SPHINX_Candidates'][$attr][$rowrolePer['snoIDs']])){ 
												$checked = 'checked="checked"' ; 
											}
								...
								{									
									...
									while($rowroleRate=mysql_fetch_assoc($resroleRate))
									{
											...
											if(isset($_SESSION['SPHINX_Candidates'][$attr][$rowroleRate['snoIDs']])){ 
												$checked = 'checked="checked"' ; 
											}
								...
								{									
									...
									if(count($explodeComm)!=0)
									{
										foreach($explodeComm as $coms)
										{
											...
											if(isset($_SESSION['SPHINX_Candidates'][$attr][$coms])){ 
												$checked = 'checked="checked"' ; 
											}
								...
								{  //Search Filter for Desired Job type 
									...
									if(count($jType)!=0)
									{
										foreach($jType as $type)
										{
											...
											if(in_array($type,$_SESSION['SPHINX_Candidates'][$attr])){ 
													$checked = 'checked="checked"' ; 
											}
								...
								{ //Search Filter for Desired Job status
									...
									if(count($jStatus)!=0)
									{
										foreach($jStatus as $status)
										{
											...
											if(in_array($status,$_SESSION['SPHINX_Candidates'][$attr])){ 
													$checked = 'checked="checked"' ; 
											}
								...
								{  //Search Filter for Skill Department
									...
									while($rowSkillsDept=mysql_fetch_assoc($resSkillsDept))
									{
										...
										if(isset($_SESSION['SPHINX_Candidates'][$attr][$rowSkillsDept['snoIDs']])){ 
											$checked = 'checked="checked"' ; 
										}
								...
								{  //Search Filter for Skill Category
									...
									while($rowSkillsCat=mysql_fetch_assoc($resSkillsCat))
									{
										...
										if(isset($_SESSION['SPHINX_Candidates'][$attr][$rowSkillsCat['snoIDs']])){ 
											$checked = 'checked="checked"' ; 
										}
								...
								{  //Search Filter for Skill Speciality
									...
									while($rowSkillsSpec=mysql_fetch_assoc($resSkillsSpec))
									{
										...
										if(isset($_SESSION['SPHINX_Candidates'][$attr][$rowSkillsSpec['snoIDs']])){ 
											$checked = 'checked="checked"' ; 
										}
								...
								if($attr=="crc_candtype")
								{
									while($row = mysql_fetch_array($result)) {
											...
											$count = $row['cnt'];
												...
												if(isset($_SESSION['SPHINX_Candidates'][$attr][$value])){ 
													$checked = 'checked="checked"' ; 
												}
												...
												if(($viewattr=="zip" || $viewattr=="areacode") && ($_GET[$viewattr.'miles']!='' &&  $_GET['radius'.$viewattr]!='')){
													...
													if($viewattr=="areacode"){
														$_SESSION['ACR_SPHINX_Candidates'][$_GET[$viewattr.'miles']][str_replace("_","",$print_value)]=$miles;
													}
												}else{
														if($c==1){
								...
								if(count($areaCodeMilesList)!=0)
								{
									...
									foreach ($areaCodeMilesList as $idA => $rowA)
									{
										...
										if($idn ==1){
											...
											if(isset($_SESSION['SPHINX_Candidates'][$attr][$idA])){ 
												$checked = 'checked="checked"' ; 
											}
											$subSqlCount = array();
											if((isset($_GET['hareacode']) && $_GET['hareacode'] !='') || in_array('hareacode',$areaCode_all))
											...
											$attrscontent .= "<li><input class='ckbox' type='checkbox' name='".$attr."' value='".$idA."' ".$checked."  onclick='javascript:deSelectChk();' /><a class='ckboxa' href='javascript:void(0);'>".replace_string($idA)."</a>";
												if($c==1){
													...
													if($viewattr=="areacode"){
														$_SESSION['ACR_SPHINX_Candidates'][$_GET[$viewattr.'miles']][str_replace("_","",$idA)]=$areaCodeMilesList[$idA][0];
													}
								...
								while($row = mysql_fetch_array($result,MYSQL_ASSOC)) {
									...
									$count = $row['cnt'];
										...
										if (isset($_GET[$attr])){ 
											...
											if(isset($_SESSION['SPHINX_Candidates'][$attr]['I|'.$value])){ 
												$checked = 'checked="checked"' ; 
											}
											if(isset($_SESSION['SPHINX_Candidates'][$attr]['E|'.$value])){ 
												$checked = 'checked="checked"' ; 
											}
										...
										if(($viewattr=="zip" || $viewattr=="areacode") && ($_GET[$viewattr.'miles']!='' &&  $_GET['radius'.$viewattr]!='')){
											...
											if($viewattr=="zip"){
												// $_SESSION['ZCR_SPHINX_Candidates'][$_GET[$viewattr.'miles']][html_tls_entities(ucfirst(replace_string($print_value)))]=$miles;
											}
											if($viewattr=="areacode"){
												$_SESSION['ACR_SPHINX_Candidates'][$_GET[$viewattr.'miles']][str_replace("_","",$print_value)]=$miles;
											}
