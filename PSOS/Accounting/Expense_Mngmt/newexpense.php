<?php
	...
	$query="SELECT emp_list.username uid, emp_list.name name,".getEntityDispName("CONCAT_WS(' ',hrcon_general.lname,hrcon_general.fname,hrcon_general.mname)","emp_list.sno")." 
	...
	AND hrcon_compen.dept IN (".$deptAccesSno.")
    ...
    if($empnames=="")
	{
		...
		while($Executeresult=mysql_fetch_row($execute))
			$searcharray[]=$Executeresult[0];
		$key = array_search($username,$searcharray); 
		if($key)
			$new_user=$username;
		else
			$new_user=$searcharray[0]; 
	}
    else
	{
        $new_user=$empnames;
	...
	{
      	$getmaxdate = "SELECT MAX(edate) FROM par_expense WHERE username='".$myrows[0]."'";
		$maxres=mysql_query($getmaxdate,$db);
		$maxdaterow=mysql_fetch_row($maxres);
		
		$getActiveAssignments ="SELECT count(1) FROM hrcon_jobs WHERE username = '".$myrows[0]."' AND pusername!='' AND ((hrcon_jobs.ustatus IN ('active','closed','cancel') AND (hrcon_jobs.s_date IS NULL OR hrcon_jobs.s_date='' OR hrcon_jobs.s_date='0-0-0' OR (DATE(STR_TO_DATE(s_date,'%m-%d-%Y'))<='".$end_date."'))) AND (IF(hrcon_jobs.ustatus='closed',(hrcon_jobs.e_date IS NOT NULL AND hrcon_jobs.e_date<>'' AND hrcon_jobs.e_date<>'0-0-0' AND DATE(STR_TO_DATE(e_date,'%m-%d-%Y'))>='".$start_date."'),1)) AND (IF(hrcon_jobs.ustatus='cancel',(hrcon_jobs.e_date IS NOT NULL AND hrcon_jobs.e_date<>'' AND hrcon_jobs.e_date<>'0-0-0' AND DATE(STR_TO_DATE(e_date,'%m-%d-%Y'))>='".$start_date."'),1))) AND hrcon_jobs.jtype!='' ORDER BY udate";
		...
		if($dateFlag)
		{
	  		if($enames=="")
			{
			    $enames="<option value='".$myrows[0]."' ".sel($new_user,$myrows[0]).">".$myrows[2]."</option>";
				$empValue = $myrows[0];
			}	
     		 else
			 {
...
.select2-result-selectable .select2-match, .select2-result-unselectable .select2-match {color: #fff !important;}
</style>
...
<table width=100% cellpadding=0 cellspacing=0 border=0 class="ProfileNewUI" align="center">
    ...
    <td class="titleNewPad">
        ...
        <tr>
			...
			<!--
			<font class=afontstyle>Enter Expense Type </font><font class=afontstyle><input type=text name=addexpense size=10 maxlength=25 tabindex="2"> <a href="javascript:doAddExpt()">Add</a></font><font class=afontstyle3> &nbsp;&nbsp;<?php if($dd[0]>0){echo "Type Already Exists.</font>";} ?></font>
			-->
            <font class=afontstyle>Select an Employee to fill the Expenses  :&nbsp;</font><select id="expemp_list" name="empnames" class=drpdwnacc  onChange="javascript:getEmp()" tabindex="2"><!-- option value="">--Select an Employee--</option --><?php echo $enames; ?></select>
			</td>
...
<input type=hidden name='empnames' id='empnames' value='' />
