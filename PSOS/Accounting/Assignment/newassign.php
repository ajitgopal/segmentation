...
<?php
	...
	$enames="";
	$query="SELECT el.username,el.name, ".getEntityDispName("el.sno","el.name")." FROM emp_list el LEFT JOIN hrcon_compen hrc ON (hrc.username = el.username) WHERE el.lstatus != 'DA' AND el.lstatus != 'INACTIVE' AND (el.empterminated!='Y' || UNIX_TIMESTAMP(IF(el.tdate='' || el.tdate IS NULL,NOW(),el.tdate))>UNIX_TIMESTAMP(NOW())) AND hrc.dept IN(".$deptAccesSno.") GROUP BY el.sno ORDER BY el.name ASC ";
	$result=mysql_query($query,$db);
	while($myrow=mysql_fetch_row($result))
	{
		if($enames=="")
		{
			$enames="<option value='".$myrow[0]."' ".sele11($new_user,$myrow[0]).">".$myrow[2]."</option>";
