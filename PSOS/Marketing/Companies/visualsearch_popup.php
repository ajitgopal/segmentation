...
*/
	...
	{
		if($_GET['querystring']!=''){
			unset($_SESSION["SPHINX_Companies"]);
		}
		//unset($_SESSION["SPHINX_Companies"]);
	...
	if($_REQUEST['reset'] == 1){
	unset($_SESSION["SPHINX_Companies"]);
	unset($_SESSION['SPHINX_Companies_sub']);
}
	$viewattr = $_GET['viewattr'];
	if(preg_match('/^(["\']).*\1$/m', $_GET['q'])) $_GET['q'] = "=".$_GET['q'];

	if(!empty($_SESSION["SPHINX_Companies"]))
	{
		$searchstr = '';
		foreach($_SESSION['SPHINX_Companies'] as $filtertype=>$filtername){	
			if(count($_SESSION['SPHINX_Companies'][$filtertype])>0)
			{
				$searchstr .= "&".$filtertype."=";
				foreach($_SESSION['SPHINX_Companies'][$filtertype] as $filters_id=>$filters_val)
				{
...
<title>Companies Attribute Search</title>
...
$(function() {
	...
	$('.ckboxa').click(function(){
		...
		<?php
		if((isset($_SESSION['SPHINX_Companies_sub']['chkZipAll']) && $_SESSION['SPHINX_Companies_sub']['chkZipAll'] !='') || (isset($_SESSION['SPHINX_Companies_sub']['chkAreacodeAll'])) )
			{
				if(isset($_SESSION['SPHINX_Companies_sub']['chkZipAll']) && $viewattr == 'zip')
				{
			?>
			...
			<?php }
			if(isset($_SESSION['SPHINX_Companies_sub']['chkAreacodeAll']) && $viewattr == 'areacode'){?>
					document.getElementById('selrow').style.display='block';
					document.getElementById('areacodeselecctall').checked=true;
				<?php }
		}
		if(isset($_GET['querystring']) && isset($_GET['query']) && $_GET['viewattr'] == 'industry')
		{ ?>
				document.getElementById('selrow').style.display='block';
...
<div class="acjbvisualsearchFilterCount">  
    <?php if($viewattr=="zip" || $viewattr=="areacode"){
			if($viewattr=="zip" ){
				if(isset($_SESSION['SPHINX_Companies_sub']['savezipCODE']) && !empty($_SESSION['SPHINX_Companies_sub']['savezipCODE']))
				{
					$zipcodesearch = $_SESSION['SPHINX_Companies_sub']['savezipCODE'];
			...
			if($viewattr=="areacode"){
				if(isset($_SESSION['SPHINX_Companies_sub']['saveareacodePSM']) && !empty($_SESSION['SPHINX_Companies_sub']['saveareacodePSM']))
				{
					$areacodesearch = $_SESSION['SPHINX_Companies_sub']['saveareacodePSM'];
