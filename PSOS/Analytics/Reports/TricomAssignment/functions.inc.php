...
function getFilters($filter_names, $filter_values,$main,$accountingExport,$deptAccesSno)
 {
    $show_filters = '';
    $filtervalues = '';
    $headers = 	array('jobtitle' => 'Job Title','assignmentid' => 'Assignment Id','department' => 'HRM Departments','modifieddate' => 'Last Exported Date');
...
eg: The last saved exported filename: Tricom-Assignment-20200927-093109-AM.csv
Date to be selected under Last Exported Date filter: 09/27/2020 09:31:09 AM"></i>';
        ...
        else if($filternames[$i] == 'assignmentid')
        {
            $value = html_tls_specialchars($filtervalues[$i]);
            $show_filters .=  '<font class="afontstyle">
            <input type="text" name="assignmentid" id ="'.$filternames[$i].'" value="'.stripslashes($value).'">   
            </font> <a href=javascript:assignmentsWindow("assignmentid")>
            <i alt="Search" class="fa fa-search"></i></a>';
        }
