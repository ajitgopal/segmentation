...
function getFilters($filter_names, $filter_values,$main,$accountingExport,$deptAccesSno)
{
    ...
    $headers = 	array('jobtitle' => 'Job Title','jobtype' => 'Job Type','Customer' => 'Customer Name','department' => 'HRM Departments','modified_date' => 'Last Exported Date');
...
eg: The last saved exported filename: Tricom-JobOrder-20200927-093109-AM.csv
Date to be selected under Last Exported Date filter: 09/27/2020 09:31:09 AM"></i>';
        ...
        else if($filternames[$i] == "jobtype")
        {
            $selectValue = $correspondingvalues_array[$filternames[$i]];
            $options = getManageListOptions("jotype",$selectValue);
            $show_filters .="<select class=drpdwne  name='select_{$filternames[$i]}' id='select_{$filternames[$i]}'>";
